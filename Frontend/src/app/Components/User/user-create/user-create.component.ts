...
export class UserCreateComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private userService: UsersService, private campusCourseService: CampusCourseService, private authService: AuthService,  private router: Router) {
  ...
  ngOnInit(): void {
    ...
    if (this.selectUser) {
      ...
      this.userCreate.patchValue(this.selectUser);
      this.userCreate.get('password')?.reset();
      this.disable = true;
    } else {
      this.title = 'Crear';
    }
  ...
  updateUser() {
    ...
    this.userService.updateUser(this.user, this.selectUser.id).subscribe(r => {
      this.Modal = true;
      this.Messages = [{
        severity: 'success',
        summary: 'Actualizado',
